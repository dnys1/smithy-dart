...
library rest_json1.rest_json_protocol.model.input_and_output_with_headers_io;
...
abstract class InputAndOutputWithHeadersIo
  ...
  factory InputAndOutputWithHeadersIo.fromRequest(
    ...
    final builder = InputAndOutputWithHeadersIoBuilder();
    if (request.headers['X-String'] != null) {
      builder.headerString = request.headers['X-String']!;
    }
    if (request.headers['X-Byte'] != null) {
      builder.headerByte = int.parse(request.headers['X-Byte']!);
    }
    if (request.headers['X-Short'] != null) {
      builder.headerShort = int.parse(request.headers['X-Short']!);
    }
    if (request.headers['X-Integer'] != null) {
      builder.headerInteger = int.parse(request.headers['X-Integer']!);
    }
    if (request.headers['X-Long'] != null) {
      builder.headerLong = _i5.Int64.parseInt(request.headers['X-Long']!);
    }
    if (request.headers['X-Float'] != null) {
      builder.headerFloat = double.parse(request.headers['X-Float']!);
    }
    if (request.headers['X-Double'] != null) {
      builder.headerDouble = double.parse(request.headers['X-Double']!);
    }
    if (request.headers['X-Boolean1'] != null) {
      builder.headerTrueBool = request.headers['X-Boolean1']! == 'true';
    }
    if (request.headers['X-Boolean2'] != null) {
      builder.headerFalseBool = request.headers['X-Boolean2']! == 'true';
    }
    if (request.headers['X-StringList'] != null) {
      builder.headerStringList.addAll(_i1
          .parseHeader(request.headers['X-StringList']!)
          .map((el) => el.trim()));
    }
    if (request.headers['X-StringSet'] != null) {
      builder.headerStringSet.addAll(_i1
          .parseHeader(request.headers['X-StringSet']!)
          .map((el) => el.trim()));
    }
    if (request.headers['X-IntegerList'] != null) {
      builder.headerIntegerList.addAll(_i1
          .parseHeader(request.headers['X-IntegerList']!)
          .map((el) => int.parse(el.trim())));
    }
    if (request.headers['X-BooleanList'] != null) {
      builder.headerBooleanList.addAll(_i1
          .parseHeader(request.headers['X-BooleanList']!)
          .map((el) => el.trim() == 'true'));
    }
    if (request.headers['X-TimestampList'] != null) {
      builder.headerTimestampList.addAll(_i1
    ...
    if (request.headers['X-Enum'] != null) {
      builder.headerEnum =
          _i4.FooEnum.values.byValue(request.headers['X-Enum']!);
    }
    if (request.headers['X-EnumList'] != null) {
      builder.headerEnumList.addAll(_i1
  ...
  factory InputAndOutputWithHeadersIo.fromResponse(
    ...
    if (response.headers['X-String'] != null) {
      builder.headerString = response.headers['X-String']!;
    }
    if (response.headers['X-Byte'] != null) {
      builder.headerByte = int.parse(response.headers['X-Byte']!);
    }
    if (response.headers['X-Short'] != null) {
      builder.headerShort = int.parse(response.headers['X-Short']!);
    }
    if (response.headers['X-Integer'] != null) {
      builder.headerInteger = int.parse(response.headers['X-Integer']!);
    }
    if (response.headers['X-Long'] != null) {
      builder.headerLong = _i5.Int64.parseInt(response.headers['X-Long']!);
    }
    if (response.headers['X-Float'] != null) {
      builder.headerFloat = double.parse(response.headers['X-Float']!);
    }
    if (response.headers['X-Double'] != null) {
      builder.headerDouble = double.parse(response.headers['X-Double']!);
    }
    if (response.headers['X-Boolean1'] != null) {
      builder.headerTrueBool = response.headers['X-Boolean1']! == 'true';
    }
    if (response.headers['X-Boolean2'] != null) {
      builder.headerFalseBool = response.headers['X-Boolean2']! == 'true';
    }
