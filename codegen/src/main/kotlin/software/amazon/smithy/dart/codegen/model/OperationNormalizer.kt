...
object OperationNormalizer {
     ...
     */
    fun transform(model: Model, services: List<ShapeId>): Model {
        ...
        val serviceOperations = services.associateWith { topDownIndex.getContainedOperations(it) }

        serviceOperations.forEach { (service, operations) ->
            validateTransform(model, service, operations)
        }

        val builder = model.toBuilder()
        serviceOperations.forEach { (_, operations) ->
